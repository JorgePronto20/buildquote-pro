...
import { created, ok, badRequest, unauthorized } from '../../utils/responses.js';
...
export async function handleAuthRoutes({ request, env, user, segments }) {
  ...
  if (request.method === 'POST' && action === 'register') {
    ...
    await env.DB.prepare('INSERT INTO users (id, name, email, password_hash, role, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)')
    ...
    await env.DB.prepare('INSERT INTO professionals (id, user_id, business_name, profession, email, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)')
    ...
    const modules = await env.DB.prepare('SELECT code FROM modules WHERE is_core = 1 OR is_active = 1').all();
    for (const mod of modules.results || []) {
      await env.DB.prepare('INSERT OR IGNORE INTO professional_modules (id, professional_id, module_code, is_enabled, created_at) VALUES (?, ?, ?, 1, ?)')
