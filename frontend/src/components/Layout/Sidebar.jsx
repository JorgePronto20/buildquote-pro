...
const items = [
  ...
  { to: '/catalog', label: 'Catálogo Pro', icon: '🧰' },
  { to: '/materials', label: 'Materiais Base', icon: '📦' },
...
export default function Sidebar({ open, onClose }) {
  return (
    <>
      {/* @section: mobile-sidebar-backdrop */}
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 w-72 transform bg-[#1e3a5f] text-white transition lg:static lg:translate-x-0 ${open ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex h-16 items-center gap-3 border-b border-white/10 px-5">
          ...
          <div>
            ...
            <p className="text-xs text-blue-100">Fase 2 · Catálogo e Aprovação</p>
          </div>
        </div>
